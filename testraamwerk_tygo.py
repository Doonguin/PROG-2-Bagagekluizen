...
def nieuwe_kluis():
    # Open "testkluizen.txt" for read and write
    safes = open("testkluizen.txt", "r+")
    safesRead = safes.readlines()
    count = 0
    nbr = 1

    usrInp = str(input("Kluis code: "))
    ...
    for line in safesRead:
        occupied.append(int(line.split(';')[0]))

    # Check every entry in the list of occupied safes and see which one is free
    while nbr in occupied:
        nbr = nbr + 1

    # Write the safe number and password to the file
...
def kluis_openen():
    ...
    # Ask the user for their safe number and passcode
    safeN = input("Kluisnummer: ")
    passCode = input("Code: ")
